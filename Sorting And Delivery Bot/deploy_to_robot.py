...
if project_info["group"] == 0:
    error("Please set your robot's group number (eg, 23) in the project_info.json file.")
...
def copy_project_folder_to_brick():
    ...
    error_checklist = dedent("""\
        Here are some things to check:

        - Make sure you have followed all the setup instructions
        - Make sure the robot is turned on
        - Make sure the robot is connected to the same Wi-Fi network and the same band (2.4 or 5 GHz) as this computer

        If this command keeps failing, try transferring the files using an alternative method.
        """)
    
    if is_windows:
        rm_cmd = f'plink -batch -l pi -pw "{password}" {robot_name} "rm -rf {robot_project_path}"'
        if command_result(rm_cmd):
            error(f"Failed to connect to brick or remove old project. {error_checklist}")
        else:
    ...
    if command_result(copy_cmd):
        error(f"Failed to copy project to brick because the copy command failed. {error_checklist}")
